local composer = require("composer")
local scene = composer.newScene()

local retornar

local function abrirPage01(self, event)
    if event.phase == "ended" or event.phase == "cancelled" then
        composer.gotoScene("page01", "slideRight", 800)
        return true
    end
end

function scene:create(event)
    local sceneGroup = self.view

    local background = display.newImageRect(sceneGroup, "assets/fundo_branco.png", display.contentWidth, display.contentHeight * 2)
    background.anchorX = 0
    background.anchorY = 0
    background.x, background.y = 0, -150


    local options1 = {
        text = "Matheus Sousa Lima \n\n Maio \nde 2022",
        x = display.contentWidth * 0.5,
        y = display.contentHeight - (display.contentHeight * 0.05),
        width = 600,
        font = native.systemFont,
        fontSize = 40,
        align = "center"
    }

    local options2 = {
        text = "Professor: Ewerton Mendonça",
        x = display.contentWidth * 0.5,
        y = display.contentHeight - (display.contentHeight * 0.42),
        width = 600,
        font = native.systemFont,
        fontSize = 60,
        align = "center"
    }

    local myText1 = display.newText(options1)
    local myText2 = display.newText(options2)

    local upelogo = display.newImageRect(sceneGroup, "assets/upe.png", 400, 270)
    upelogo.x = display.contentWidth * 0.5
    upelogo.y = display.contentHeight * 0.2

    retornar = display.newImageRect(sceneGroup, "assets/seta_azul.png", 120, 90)
    retornar.x = display.contentWidth * 0.1
    retornar.y = display.contentHeight + 250

    transition.to( retornar, { rotation=180 } )

    sceneGroup:insert(background)
    sceneGroup:insert(myText1)
    sceneGroup:insert(myText2)
    sceneGroup:insert(retornar)
    sceneGroup:insert(upelogo)

    local bgMusic = audio.loadStream("assets/vento.mp3")
    audio.reserveChannels(1)
    audio.setVolume( 0.5, { channel=1 } )
    audio.play( bgMusic, { channel=1, loops=-1 } )
end

function scene:show(event)
    local sceneGroup = self.view
    local phase = event.phase

    if phase == "will" then
    elseif phase == "did" then
        retornar.touch = abrirPage01
        retornar:addEventListener("touch", retornar)
    end
end

function scene:hide(event)
    local sceneGroup = self.view
    local phase = event.phase

    if event.phase == "will" then
        retornar:removeEventListener("touch", retornar)

    elseif phase == "did" then

    end
end

function scene:destroy(event)
    local sceneGroup = self.view

end

scene:addEventListener("create", scene)
scene:addEventListener("show", scene)
scene:addEventListener("hide", scene)
scene:addEventListener("destroy", scene)

return scene
